...
# =====================================================================
# Interactive Testing and Verification Demo
# =====================================================================
if __name__ == "__main__":
    print("=" * 70)
    print("Dino Vision - Screen Capture Interactive Diagnostic")
    print("=" * 70)
    print("Controls:")
    print("  - Move/Resize Tkinter capture window to position over game.")
    print("  - GUI Buttons:")
    print("      ▶ Play   : Start/Resume feed (continuous frames)")
    print("      ⏸ Pause  : Pause feed (stop continuous frames)")
    print("      ✕ Cancel : Cleanly close window (no crash / no Task Manager hang)")
    print("  - OpenCV Preview Window Shortcuts (when focused):")
    print("      'p' : Toggle Play / Pause")
    print("      'r' : Toggle Video Recording (saves to logs/recordings/)")
    print("      's' : Save snapshot (saves to logs/snapshots/)")
    ...
    window_name = "Dino Vision - Live Stream Preview"
    recording = False
    frame_count = 0
    ...
    try:
        for frame in capture.start(fps_limit=30):
            ...
            if cv2 is not None:
                ...
                # Status watermark on preview
                elapsed = time.time() - start_time
                fps = frame_count / elapsed if elapsed > 0 else 0
                status_text = f"FPS: {fps:.1f} | Frame: {frame_count}"
                if capture.video_recorder and capture.video_recorder.is_recording:
                    status_text += " [REC ●]"
                ...
                cv2.putText(
                    ...
                    0.6,
                    (0, 0, 255) if (capture.video_recorder and capture.video_recorder.is_recording) else (0, 255, 0),
                    2
                )
                ...
                if key == ord('q') or key == 27:  # 'q' or ESC
                    logger.info("Exit requested from preview window.")
                    capture.close()
                    break
                elif key == ord('p'):
                    ...
                    if capture.video_recorder and capture.video_recorder.is_recording:
                        ...
                        print(f">> Stopped recording: {path}")
                    else:
                        capture.start_recording()
                        print(">> Started recording stream to video...")
                elif key == ord('s'):
                    snapshot_path = os.path.join(
                        "logs", "snapshots", f"snapshot_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
                    )
                    capture.save_frame(frame, snapshot_path)
        ...
        print("[Done] Program finished cleanly with 0 crashes.")